...
export default function App() {
  ...
  const initializeSimulation = (width: number, height: number) => {
    ...
    // 1. Generate 1500 text particles
  ...
  const updateAndRender = (dt: number) => {
    ...
    // Double glow back-buffer (Ethereal luxury light layer)
    ctx.globalCompositeOperation = 'screen';
    for (let i = 0; i < s.activeCount; i += 6) {
      ...
      const alphaGlow = Math.max(0.1, (1.0 - distToTarget / 400)) * p.alpha * 0.16 * intensity;
      
      ctx.font = `300 ${p.fontSize * 1.8}px "Cormorant Garamond", serif`;
      ctx.fillStyle = s.colors.glowMain;
      ctx.shadowColor = s.colors.glowMain;
    ...
    for (let i = 0; i < s.activeCount; i++) {
      ...
      if (s.mouse.active) {
        ...
        if (mdist < 130) {
          ...
          forceX += (mdx / (mdist || 1)) * push;
          forceY += (mdy / (mdist || 1)) * push;
        }
      }
      ...
      s.pulses.forEach(pulse => {
        ...
        if (Math.abs(pdist - pulse.radius) < 45) {
          const waveStrength = (1.0 - Math.abs(pdist - pulse.radius) / 45);
          const force = waveStrength * 7.5 * (1.0 - pulse.radius / pulse.maxRadius) * pulse.strength;
          forceX += (pdx / (pdist || 1)) * force;
          forceY += (pdy / (pdist || 1)) * force;
        }
      });
      ...
      ctx.font = `200 ${p.fontSize}px "${isPrimaryText ? 'Cormorant Garamond' : 'Inter'}", serif`;
    ...
    const grainBuffer = ctx.createImageData(canvas.width, canvas.height);
    const grainData = grainBuffer.data;
    const len = grainData.length;
    const frameToggle = Math.floor(s.timeElapsed * 60) % 2;
    if (frameToggle === 0) {
      ...
      ctx.putImageData(grainBuffer, 0, 0);
    }
